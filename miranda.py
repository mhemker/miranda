    ...
    def getHostInfo(self,xmlData,xmlHeaders,index):
        ...
        if index >= 0 and index < len(self.ENUM_HOSTS):
            try:
                ...
                print(type(xmlHeaders))
                self.ENUM_HOSTS[index]['serverType'] = xmlHeaders['Server'] # xmlHeaders.getheader('Server')
...
#            hp.parseSSDPInfo(hp.listen(1024,False),False,False)
            result = hp.listen(1024,False)
            if type(result) is bool:
                print('Stopped listening')
                break;
            hp.parseSSDPInfo(result,False,False)
